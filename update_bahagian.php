 ...
 if(isset($_GET['sectID']))
{
  ...
  $sectID = urldecode(secured_decrypt($_GET['sectID']));
  $sql = "SELECT * FROM section WHERE sectID = $sectID";
  //echo $sql;
...
$query = "UPDATE section SET sectName = '$sectName' WHERE sectID = '$sectID'";
//echo $query;
...
header("Location: bahagian.php?op=errkod");
}
else
{
  echo "<script>
...
header("Location: bahagian.php?op=success");
...
else if(isset($_POST['padam']))
{
  ...
  $query = "DELETE FROM section WHERE sectID = $sectID";
  //echo $query;
  if (!mysqli_query($conn, $query))
  {
    echo "<script>
  ...
  header("Location: bahagian.php?del=errkod");
  }
  else
  {
    echo "<script>
  ...
  header("Location: bahagian.php?del=success");
        ...
        <div class="container-fluid">
          ...
          <div class="shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-info">KEMASKINI BAHAGIAN</h6>
            </div>
            <div class="card-body">
            <form name="update_bahagian" class="user" method="post" action = "update_bahagian.php">
            <div class="form-group">
                    <input type="hidden" class="form-control form-control-user" name="sectID" placeholder="ID Bahagian" value="<?php echo $row['sectID']; ?>" required >
            </div>
            <div class="form-group">
                <input type="text" class="form-control form-control-user" name="sectName" placeholder="Nama Bahagian" value="<?php echo $row['sectName']; ?>" required >
            </div>
          </div>
