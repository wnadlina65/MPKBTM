...
if(isset($_POST['simpan']))
{
  ...
  $duplicate = "SELECT deptName FROM department WHERE deptName = '$deptName'";
  $check = mysqli_query($conn,$duplicate);
  $checkrows = mysqli_num_rows($check);

  if ($checkrows > 0)
  {
    header("Location: Tambah_jabatan.php?op=errkod");
    return false;
  }
  else
  {
...
header("Location: Tambah_jabatan.php?op=errkod");
  ...
  header("Location: Tambah_jabatan.php?op=success");
  }
        ...
        <div class="container-fluid">
          ...
          <div class="shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-info">TAMBAH JABATAN</h6>
            </div>
            <div class="card-body">
            <form name="login" class="user" method="post" action = "Tambah_jabatan.php">
                <div class="form-group">
                    <input type="text" class="form-control form-control-user" name="deptName" placeholder="Nama Jabatan" required>
                  </div>
                      </div>
                    </div>
                  <div class="form-group">
